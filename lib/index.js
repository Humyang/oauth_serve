...
router.post('/login',async function(ctx){
    ...
    let login = await login(ctx,username,password)
...
var  md5 = require('md5')
router.post('/oauth_login',async function (ctx) {
    debugger
    // 执行注册流程
    let nurl = URL.parse(ctx.header.referer,true)
    // let nurl2 = path(ctx.header.referer)
    //
    ...
    let resError = ""
    try{
        res = await login(ctx,username,password)
    }catch(err){
        resError = err
    }
    if(resError){
        ...
        // 重定向到指定 Url，并返回 Token
        
        let redirectUrl = nurl.query.r
        if(!redirectUrl){
            await ctx.render('login',{
                errorMsg:"缺少重定向参数 '?r'"
            });
            ...
            let token = res.token
            ctx.redirect(redirectUrl+'?t='+token);  
        }
    }
...
router.get('/oauth_login',async function (ctx) {
    // console.log('login')
    // debugger
    // ctx.request.query
    // const users = [{ name: 'Dead Horse' }, { name: 'Jack' }, { name: 'Tom' }];
...
console.log('Running a API server at localhost:'+PORT);